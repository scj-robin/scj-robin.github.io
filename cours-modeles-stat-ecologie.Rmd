---
title: "Modèles à variables latentes pour l'écologie"
output: 
  html_document:
   toc: false
   logo: images/LogoSULPSM.png
   number_sections: false
   highlight: tango
   css: style.css
   includes:
     before_body: extlogo.html
---

<br>

## Modèles à variables latentes pour l'écologie

L'écologie s'intéresse aux relations que les espèces vivantes entretiennent entre elles et avec leur milieu. L'analyse et la compréhension de ces interactions passe fréquemment par une modélisation statistique impliquant des variables latentes (c'est-à-dire non observées) visant à décrire les structures et les processus qui sous-tendent ces interactions.

L'objectif de ce cours est de présenter certains de ces modèles comme les modèles de distributions (jointes) d'espèces ou les modèles de réseaux écologiques. Les modèles les plus simples sont des modèles linéaires généralisés, éventuellement mixtes. Les modèles plus complexes posent des problèmes d'inférence spécifiques qui peuvent être surmontées grâce à des généralisations de l'algorithme EM reposant sur des approximations variationnelles. Un des objectifs principaux de ce cours est la bonne compréhension de tels modèles et la définition d'un algorithme permettant d'en inférer les paramètres.

Nous utiliserons également la représentation de ces modèles selon le formalisme des modèles graphiques qui permettent de représenter la structure de dépendance entre les différentes variables (observées ou latentes) et de d’anticiper la complexité de l’algorithme d’inférence. Cette représentation est par ailleurs pertinente pour traiter le problème de l’inférence de réseaux écologiques, dans lequel il s’agit notamment de distinguer entre interactions directes ou indirectes entre les espèces.

Certains des modèles présentés seront mis en oeuvre lors de séances de travaux dirigés sur machine. Ce cours vise notamment à apprendre à définir un modèle pertinent pour répondre à une question écologique, concevoir un algorithme permettant d'en estimer les paramètres et implémenter cet algorithme.

#### Prérequis 
Notions fondamentales de probabilités et statistique, connaissance du langage R.

#### Thèmes abordés

- Modèles à variables latentes, modèles graphiques.
- Modèles de distribution d'espèces, modèles de réseaux écologiques.

----

### Déroulement pour l'année 2025-26

#### Planning

Les enseignements auront lieu les **lundis matin de 9h00 à 12h00** 

Les cours et les séances de TD se tiendront 

* en **salle 24-34/103** jusqu'au 23 février, 
* puis en **salle 14-24/107** à partir du 09 mars.

<!-- Les **séances de TD** auront lieu les 26 janvier (salle Atrium 123), 16 février (salle Atrium 115) et 23 mars (salle Atrium 115). -->

Le planning est également disponible [ici](https://ms2a.lpsm.paris/enseignements/agenda/). 

----

#### Supports de cours

Des notes de cours sont disponibles [ici](https://scj-robin.github.io/notes/ModelsHiddenStruct-Biology-29jan21.pdf), 
un projet de livre sur l'inférence des modèles à variables latentes est disponible [ici](https://scj-robin.github.io/notes/DGR25Draft.pdf), 
et les supports d'une série de cours doctoraux sur certains des modèles présentés en cours sont disponibles [ici](https://scj-robin.github.io/lecture-slides.html).

#### Exemples présentés en cours

* Cours 1 : rappel sur les modèles linéaires généralisés :
[Vitesse de courses d'herbivores et de carnivores](examples/CarniHerbiLM.html), 
[Présence/absence de flétan en mer de Barents](examples/BarentsGLMbernoulli.html), 
[Nombre de partenaires chez une espèce de crabe](examples/HorseshoeGLMpoisson.html)

* Cours 2 : modèles à variables latentes :
[Présence et abondance de la morue en mer de Barents](examples/BarentsGLMzip.html), 
[Evolution de la masse des mammifères carnivores](examples/CarnivoreBodymass.html), 
[Abondance de la plie en mer de Barens (avec surdispersion)](examples/BarentsGLMnegbin.html) 

* Cours 3 : modèles de Markov caché :
[Déplacement d'un élan](examples/ElkMovementFull.html), 
[Déplacement d'un zèbre](examples/PEL19-zebra.html) 

* Cours 4 : modèles gaussiens multivariés :
[ACP pour des traits morphologiques](examples/CESTES-PCA.html), 
[ACP phylogénétique pour des traits morphologiques](examples/Lizards-PhyloPCA.html) ; \
Modèles pour l'écologie des communautés :
[Modèle de distribution jointe d'espèces](examples/BarentsPLNintro.html), 
[Modèle de réseau d'interactions](examples/KarateSBM.html), 

* Cours 4 : modèles de réseaux d'interactions :
[Modèl à blocs stochastiques (SBM) entre espèces d'arbres](examples/TreeInteractions-SBM.html), 
[Modèl à blocs latents (LBM) entre espèces d'arbres et de champignons](examples/FungiTreeInteractions-LBM.html)  ; \
Modèles de distribution jointe d'espèces : 
[Modèle Poisson log-normal (PLN) pour des espèces de poisson](examples/BarentsPLN.html) 

<!-- ####### Données de réseaux -->

<!-- ####### Données d'abondances -->

#### TD : Sujets et données 

##### TD1 : Modélisation d'abondance par des modèles de Poisson avec surcroit de zéros

Le [sujet](td/TD1.pdf) et les [données](td/GoldenRedfish.csv).

##### TD2 : Analyse d’un déplacement animal par chaîne de Markov cachée

Le [sujet](td/TD2.pdf) et les [données](td/dryad_zebra.csv).
Code pour la récurrence forward-backward en [R](td/Mouvement-HMM-FW.R) et en [python](td/Mouvement-HMM-FW.ipynb).

<!-- ##### TD3 : -->

----

### Annales des examens passés

[Sujet](https://scj-robin.github.io/examples/Examen2023-Sujet.pdf) et [Corrigé](https://scj-robin.github.io/examples/Examen2023-Corrige.pdf) 2022-23 

[Sujet](https://scj-robin.github.io/examples/Examen2025-Sujet.pdf) et [Corrigé](https://scj-robin.github.io/examples/Examen2025-Corrige.pdf) 2024-25 

